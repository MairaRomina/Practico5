...
from flask import Flask, redirect, request, render_template, Response, flash, get_flashed_messages, url_for
...
def receta ():
    if usuario_actual == -1:
        return redirect(url_for('login')) #soluciono que el usuario se loguee si o si
    if request.method == 'POST':
        if not request.form['tiempo'] or not request.form['nombre'] or not request.form['descripcion']:
            return render_template('error.html', error='Contenido no ingresado')
        else:
            nueva_receta = Receta(nombre = request.form['nombre'], tiempo = request.form['tiempo'], fecha = datetime.today(), elaboracion = request.form['descripcion'], cantidadmegusta = 0, usuarioid = usuario_actual.id)
...
def ingrediente ():
    i = 0
    limite = int(request.form['ing'])
    if request.method == 'POST':
            for i in range(limite):
                if request.form['nombreIngrediente'+str(i)] is None and request.form['cantidad'+str(i)] is None and request.form['unidad'+str(i)] is None:
                    return render_template('error.html', error='Contenido no ingresado')
                    #i += 1
                else:
